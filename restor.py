...
class Drink(Dish): #напиток
    def __init__(self, name, ingredients, base_price, volume_ml):
        super().__init__(name, ingredients, base_price)
        self._volume_ml = volume_ml 

    def info(self):
        return f"«Напиток: {self._name}, объем: {self._sweetness}мл, цена {self.total_cost():.2f} сом"  
...
class Restaurant:
    def __init__(self, name):
        ...
        self.__income = 0.0 #доход 

    @property
    def income(self):
        return self.__income

    # продаёт блюдо, увеличивает доход, убирает из меню
    ...
    def menu(self): 
        return [d for d in self.kitchen.all_dishes()]

    # доход и количество оставшихся блюд 
    def status(self): 
        return {
            'Ресторан':self.name,
            'Доход':self.income,
